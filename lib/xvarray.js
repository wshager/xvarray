...
function join($a) {
	...
	return _xvseq.seq($a.reduce(function(pre,cur){
		var v = _xvseq._first(cur);
		//if(f && f._isSeq) console.log("join:",f.first(),_immutable.Map.isMap(f.first()))
		return pre.concat(v);
	},_immutable.List()));
}
...
function subarray($a,$s,$e) {
	var s =  _xvseq._first($s) || 1, e = _xvseq._first($e);
...
function append(...args) {
	...
	var ret;
	try {
		ret = insert.isEmpty() ? a : a.push(insert);
	} catch(e){
		console.log("arrerr", e);
	}
